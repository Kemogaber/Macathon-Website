...
_SYSTEM_PROMPT = (
    ...
    "All y-values MUST be plain numbers (e.g. 1234, not \"1,234\" or "
    "\"$1.2k\" or \"50%\"). Strip commas, currency symbols and percent "
    "signs before emitting. For pie charts: `x` is the category/label "
    "field and `y` is a single numeric field; every data row must have a "
    "positive number for `y` or the slice will not render. Only include "
    "rows that have a real value for the y field — skip totals, blanks, "
    "and header rows. Briefly describe the chart in prose alongside the "
    "block.\n\n"
    "## Cell fixes\n"
