...
def rotate_page(job_id: str, page_index: int, degrees: float) -> Job:
    """Rotate a page image by any angle on disk; clear stale detections.

    `degrees` is positive-clockwise. The output canvas grows to fit the
    rotated image — corners are filled white so they don't introduce a
    black border that detection might mistake for a table edge. 90/180/270
    use cv2.rotate (lossless); other angles use warpAffine with cubic
    interpolation. Detections are cleared because their coordinates don't
    apply to the rotated image; user re-runs Parse for that page.
    """
    job = _JOBS.get(job_id)
    if job is None:
        raise ValueError("job not found")
    ...
    if deg in (90.0, 180.0, 270.0):
        ...
        h, w = bgr.shape[:2]
        # cv2 takes counter-clockwise positive; flip sign for our convention.
...
def run_recognize(job_id: str, confirmed: list[dict]) -> None:
    ...
    job.error = None
    job.cancel_requested = False
    start_index = len(job.tables)  # append, not reset — supports per-page recognize
    ...
    with _LOCK:
        if cancelled:
            pass  # don't count cancellation as failure
        elif failures:
    ...
    _record_metric({
        "ts": time.time(),
        "job_id": job_id,
        "status": "cancelled" if cancelled else ("error" if failures else "done"),
        "duration_ms": int((time.monotonic() - started) * 1000),
        "table_count": len(job.tables) - start_index,
...
def _cgroup_memory() -> tuple[int, int] | None:
    """Return (used_bytes, limit_bytes) from cgroup, or None if unavailable.

    Inside an HF Space container, /proc/meminfo reports the host's RAM (the
    box is shared with many other Spaces, so it always looks ~88% full and
    ~256GB total). The container's actual quota lives in cgroup. cgroup v2
    is what HF uses on modern hardware; v1 is the fallback for older hosts.
    """
    try:
        # cgroup v2
    ...
    try:
        ...
        # v1 uses an enormous sentinel for "unlimited"; ignore that case.
        if 0 < limit < (1 << 62):
            return used, limit
    except Exception:
        pass
...
def get_health() -> dict:
    ...
    RAM numbers prefer cgroup (the container's quota — what the HF dashboard
    shows) over /proc/meminfo (the host's total — meaningless on a shared
    host). Falls back to psutil if cgroup files aren't readable.
    """
    info: dict = {
        "status": "ok",
        "uptime_s": int(time.time() - _METRICS["started_at"]),
